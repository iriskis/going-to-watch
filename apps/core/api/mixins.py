...
class UpdateModelWithoutPatchMixin:
    ...
    Disables `PATCH` methods.
    Frontend doesn't use it, we write serializers expecting to have all data we
    need to do the thing. But `PATCH` allows bypassing `required` restriction.
